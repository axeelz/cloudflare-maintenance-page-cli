...
const cloudflareConfig = Effect.all([
  ...
  Config.string("WORKER_SCRIPT_NAME").pipe(Config.withDefault("cf-maintenance-worker")),
]).pipe(
...
export class CloudflareService extends Effect.Service<CloudflareService>()("app/CloudflareService", {
  effect: Effect.gen(function* () {
    ...
    const getZoneDomain = Effect.tryPromise({
      try: async () => {
        const zoneDetails = await client.zones.get({ zone_id: config.zoneId });
        return zoneDetails.name;
      },
      catch: (error) => new CloudflareError({ message: `Failed to get zone domain: ${error}` }),
    });

    const enabledPattern = (zoneDomain: string) => `*.${zoneDomain}/*`;
    const disabledPattern = (zoneDomain: string) => `*.${zoneDomain}/maintenance`;
    ...
    const getRoutes = Effect.tryPromise({
      ...
      catch: (error) => new CloudflareError({ message: `Failed to get routes: ${error}` }),
    });

    const getRouteWithPattern = (pattern: string) =>
      pipe(
        getRoutes,
        Effect.map((routes) => routes.find((route) => route.pattern === pattern)),
        Effect.flatMap((route) =>
          route
            ? Effect.succeed(route)
            : Effect.fail(new CloudflareError({ message: `No route found with pattern: ${pattern}` }))
        )
      );
    ...
    const createRoute = (newPattern: string) =>
      Effect.tryPromise({
        ...
        catch: (error) => new CloudflareError({ message: `Failed to create route: ${error}` }),
      });
    ...
    const updateRoute = (routeId: string, newPattern: string) =>
      Effect.tryPromise({
        ...
        catch: (error) => new CloudflareError({ message: `Failed to update route: ${error}` }),
      });
    ...
    const setupScript = (options: MaintenanceOptions) =>
      Effect.tryPromise({
        ...
        catch: (error) => new CloudflareError({ message: `Failed to setup script: ${error}` }),
      }).pipe(
        Effect.andThen(
          pipe(
            getZoneDomain,
            Effect.andThen((zoneDomain) => createRoute(disabledPattern(zoneDomain)))
          )
        )
      );
    ...
    const enableMaintenance = Effect.gen(function* () {
      ...
      const route = yield* getRoutes.pipe(Effect.flatMap(() => getRouteWithPattern(disabledPattern(zoneDomain))));
      yield* updateRoute(route.id, enabledPattern(zoneDomain));
    });

    const disableMaintenance = Effect.gen(function* () {
      const zoneDomain = yield* getZoneDomain;
      const route = yield* getRoutes.pipe(Effect.flatMap(() => getRouteWithPattern(enabledPattern(zoneDomain))));
